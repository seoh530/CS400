...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
  ...
  /**
   * This is first tester method to test rotate method. 
   * @return true if all test passed, otherwise false
   */
  public static boolean test1() {
    ...
    Node<Integer> root = tree.root; // 7
    Node<Integer> leftChild = root.context[1]; // null
    Node<Integer> rightChild = root.context[2]; // 14

    // CASE1: Check NullPointerException: there is No leftChild
    try {
    ...
    // CASE2: Check IllegalArgumentException:
    try {
      tree.rotate(rightChild, root);
      // CASE3: check rotation is correct
      // After rotation:   14
      //                   / \
      //                  7  18                     
      String expected = "[ 14, 7, 18 ]";
    ...
    Node<Integer> root2 = tree.root; //14
    Node<Integer> leftChild2 = root2.context[1]; //7
    // Node<Integer> rightChild2 = root2.context[2];

    // CASE4: right rotation after insert 2 values
    try {
      tree.rotate(leftChild2, root2);

      // CASE5: check rotation is correct
      // After rotation:    7
      //                   /  \
      //                  1   14
      //                   \    \
  ...
  /**
   * This is second tester method to test rotate method. 
   * @return true if all test passed, otherwise false
   */
  public static boolean test2() {
    ...
    // Perform rotations
    Node<Integer> root = tree.root; // 7
    Node<Integer> leftChild = root.context[1]; // 3
    
    try {
      tree.rotate(leftChild, root); // Perform right rotation
      // After rotation:       3
      //                      / \
      //                     1   7
      //                    /
      //                   2
      String expected = "[ 3, 1, 7, 2 ]";
      String actual = tree.toLevelOrderString();
      if (!expected.equals(actual)) {
        System.out.println("rotation is incorrect");
        return false;
    ...
    try {
    //CASE2: insert 9 and peform left rotation
    tree.insert(9);
    
    // Perform rotations
    root = tree.root; // 3
    Node<Integer> rightChild = root.context[2]; // 7
  ...
  /**
   * This is third tester method to test rotate method. 
   * @return true if all test passed, otherwise false
   */
  public static boolean test3() {
    ...
    Node<Integer> leftChild = tree.root.context[1];  // 30
    Node<Integer> rightChild = tree.root.context[2];  // 70
    
    tree.rotate(leftChild, root); //right rotation
    ...
    String expected = "[ 30, 20, 50, 40, 70, 60, 80 ]";
    String actual = tree.toLevelOrderString();
    if (!expected.equals(actual)) {
      System.out.println("rotation is incorrect");
      return false;
    ...
    //CASE2: left rotation after right rotation
    root = tree.root; //30
    leftChild = tree.root.context[1];  // 20
    rightChild = tree.root.context[2];  // 50
    ...
    String expected2 = "[ 50, 30, 70, 20, 40, 60, 80 ]";
    String actual2 = tree.toLevelOrderString();
